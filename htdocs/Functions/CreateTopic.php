...
if (isset($_POST['topic_submit'])) {
	...
	if (($_POST['topic_title'] == "") && ($_POST['topic_content'] == "")) {
		...
		$sql = "INSERT INTO topics (forum_id, topic_title, topic_creator, topic_date, topic_reply_date) VALUES ('".$cid."', '".$title."', '".$creator."', now(), now())";
		...
		$sql2 = "INSERT INTO posts (forum_id, topic_id, post_creator, post_content, post_date) VALUES ('".$cid."', '".$new_topic_id."', '".$creator."', '".$content."', now())";
		// Execute the INSERT query
		$res2 = mysql_query($sql2) or die(mysql_error());
		// Update the forum category associated with this new topic
		$sql3 = "UPDATE forums SET last_post_date=now(), last_user_posted='".$creator."' WHERE id='".$cid."' LIMIT 1";
		// Execute the category UPDATE query
		$res3 = mysql_query($sql3) or die(mysql_error());
		// Check to make sure all the required queries have been executed
		if (($res) && ($res2) && ($res3)) {
			header("Location: ../ViewTopic.php?cid=".$cid."&tid=".$new_topic_id);
		} else {
