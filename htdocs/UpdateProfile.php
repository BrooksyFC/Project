...
if(!isset($_SESSION['username'])) header("location:Design.php");
?>
<html>
<head>
...
<div id="Content"><h2>Update Your Profile</h2>
<?php $usersData = getUserData(getId($_SESSION['username'])); ?>

<strong>Update Your Name</strong>
<form action="UpdateProfile.php?update=name" method="POST">
	Fullname: <input type="text" maxlength="30" name="fullname" value="<?php echo $usersData['fullname']; ?>" />
...
<?php
	if(isset($_GET['update']) && $_GET['update']=="name")
	{
		...
		if(empty($errors))
		{
			if(updateFullname($usersData['id'],$fullname))
				echo "Updated!";
			else
				echo "Error!";
		}
			...
			foreach($errors as $e)
				echo $e."<br>"; //loop through errors array and print each error
	}
?>
<hr>
<strong>Update Your Profile Picture</strong><br>
<form action="UpdateProfile.php?update=profile" method="POST" enctype="multipart/form-data">
	<input type="file" name="profile">	
	<input type="submit" name="profileSubmit" value="Update">
</form>
<?php 
	if(isset($_GET['update']) && $_GET['update']=="profile")
	{
		...
		if ($size > 2097152)
			$errors[] = "File Size Too Big. 2MB Limit";
		
		if(empty($errors))
		{
			if(updateProfile($usersData['id'],$_FILES["profile"]["tmp_name"],$ext))
				echo "Uploaded!";
			else
				echo "Error!";
		}
			...
			foreach($errors as $e)
				echo $e."<br>"; //loop through errors array
	}
?>
</div>
</div>
